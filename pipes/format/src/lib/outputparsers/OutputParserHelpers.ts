...
export const injectOutputParser = (
  ...
  chain: LLMChain<
    string | object | BaseLanguageModel<any, BaseLanguageModelCallOptions>
  >,
    ...
    if (chain.prompt instanceof PromptTemplate) {
      ...
      pt.promptMessages.forEach((msg) => {
        if (msg instanceof SystemMessagePromptTemplate) {
          (msg.prompt as any).partialVariables = {
      ...
      chain.prompt.examplePrompt.partialVariables = {
        format_instructions: formatInstructions,
      };
      chain.prompt.examplePrompt.template =
        chain.prompt.examplePrompt.template + '\n{format_instructions}';
    }
