...
import { z } from 'zod';
import {INode, INodeData, INodeParams} from "types";
import {convertSchemaToZod} from "../../utils/convertSchemaToZod";
import {getBaseClasses} from "../../utils/getBaseClasses";
...
class StructuredOutputParser implements INode {
  ...
  async init(nodeData: INodeData): Promise<any> {
    const {jsonStructure, autofixParser} = nodeData.inputs as {autofixParser?: boolean, jsonStructure: string} || {
        autofixParser: true,
        jsonStructure: "{}",
        };
    
    const autoFix = autofixParser ?? true;
  
    try {
      const structuredOutputParser =
        LangchainStructuredOutputParser.fromZodSchema(
          z.object(convertSchemaToZod(jsonStructure))
        );
