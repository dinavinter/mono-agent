...
type AIDeveloperSandboxParams={
  ///@ example 'Write a function that takes a list of strings and returns the longest string in the list.'
  task: string 
}

export async function AIDeveloperSandbox({assistant_id, openai }:{assistant_id:string, openai:OpenAI}) {
   const AI_ASSISTANT_ID = assistant_id;

  return async ({task}: AIDeveloperSandboxParams) => {
